...
try {
    ...
    supabaseClient = createClient(
        'https://searruwutinbfqhqxdjo.supabase.co',
        'sb_publishable_YyD0-xZDrArmFh7V0qtSZg_lUxLuTUe'
    );
} catch (e) {
    console.warn('Supabase failed to initialize:', e);
}

// Fetch (or create) a level and return its id + current average.
// Lives outside DOMContentLoaded so fetches can start before the DOM is ready.
async function getLevelData(blue, red) {
    if (!supabaseClient) return { levelId: null, avg: 0 };
    try {
        let { data: level } = await supabaseClient
        ...
        if (!level) {
            const { data: newLevel } = await supabaseClient
                .from('ftb_levels')
                .insert({ blue_x: blue.x, blue_y: blue.y, red_x: red.x, red_y: red.y })
                .select('id').single();
            level = newLevel;
        }

        if (!level) return { levelId: null, avg: 0 };

        const { data: solves } = await supabaseClient
            .from('ftb_solves').select('solve_time').eq('level_id', level.id);

        const avg = solves?.length
            ? Math.round(solves.reduce((s, r) => s + r.solve_time, 0) / solves.length) : 0;

        return { levelId: level.id, avg };
    } catch (e) {
        console.warn('Supabase fetch failed:', e);
        return { levelId: null, avg: 0 };
    }
}

// Kick off the first fixed level fetch immediately — well before DOMContentLoaded fires.
const firstLevelPromise = getLevelData({ x: 1, y: 1 }, { x: 2, y: 2 });
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    let gameCount = 0;
    const fixedLevels = [
        { bluePoint: { x: 1, y: 1 }, redPoint: { x: 2, y: 2 } },   // easy:   scale by 2
        { bluePoint: { x: 2, y: 1 }, redPoint: { x: -1, y: 2 } },  // medium: 90° rotation
        { bluePoint: { x: 3, y: 2 }, redPoint: { x: 1, y: -4 } },  // hard:   mixed transform
    ];
    let dragging = null;
    let gameWon = false;
    let timer = null;
    ...
    function generateValidPoints() {
        if (gameCount < fixedLevels.length) {
            const level = fixedLevels[gameCount++];
            return { ...level, solution: hasIntegerSolution(level.bluePoint, level.redPoint) };
        }
        gameCount++;
    ...
    async function fetchOrCreateLevel(blue, red) {
        const isFirstLevel = blue.x === 1 && blue.y === 1 && red.x === 2 && red.y === 2;
        const { levelId, avg } = isFirstLevel
            ? await firstLevelPromise
            : await getLevelData(blue, red);

        currentLevelId = levelId;
        document.getElementById('avgTime').innerText = avg > 0 ? `Average: ${avg}s` : 'Average: 0';
    }

    async function updateAvgDisplay() {
        if (!supabaseClient || !currentLevelId) return;
        const { data: solves } = await supabaseClient
            .from('ftb_solves').select('solve_time').eq('level_id', currentLevelId);
        const avgEl = document.getElementById('avgTime');
        if (solves?.length) {
            const avg = Math.round(solves.reduce((s, r) => s + r.solve_time, 0) / solves.length);
            avgEl.innerText = `Average: ${avg}s`;
        } else {
            avgEl.innerText = 'Average: 0';
        }
    }

    async function saveSolve() {
        if (!supabaseClient || !currentLevelId || hasClickedSolve || hasSavedThisGame) return;
        hasSavedThisGame = true;
        try {
            await supabaseClient
                .from('ftb_solves')
                .insert({ level_id: currentLevelId, solve_time: elapsedTime });
            updateAvgDisplay();
        } catch (e) {
            console.warn('Failed to save solve:', e);
        }
    }
    ...
    function handleReset() {
        ...
        stopTimer();
        startTimer();
        document.getElementById('avgTime').innerText = 'Average: 0';
        fetchOrCreateLevel(bluePoint, redPoint);
    }
